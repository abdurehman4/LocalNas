...
const Header = () => {
  ...
  return (
    ...
    <header className='sticky flex flex-row justify-right lg:flex-row top-0 z-10 bg-white outline outline-1 outline-slate-200'>
      ...
      {/* { */}
      {/* // navBar && ( */}
      <button type="button" className='lg:hidden' onClick={()=>{setNavBar(!navBar)}} ><Image className="lg:hidden h-8 w-7" alt='' src={bar} key="hek" /></button>
        <div className={navBar? "mx-5 md:mx-10 absolute origin-top-right	 transition w-fit lg:hidden ease-in-out duration-500 right-0 z-10 mt-50 top-16 w-56 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none ":"hidden mx-5 md:mx-10 absolute origin-top-right	 transition w-fit lg:hidden ease-in-out duration-500 right-0 z-10 mt-50 top-16 w-56 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"} role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
