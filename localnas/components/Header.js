...
const Header = () => {
  ...
  return (
    // <ClientComponent>
    <header className='sticky flex flex-row md:flex-row top-0 z-10 bg-white outline outline-1 outline-slate-200'>
      <Link href='/'>
      ...
      <Link href=''><Image className="md:hidden h-8 w-7" alt='' src={bar} key="hek" onClick={() => setNavBar(!navBar)} /></Link>
      {
        navBar && (
          <div className="absolute ease-in-out duration-300 right-0 z-10 mt-50 top-16 w-56 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
